...
from model.module.Attention import SelfAttentionTransformer
...
class SEConv2dBlock(nn.Module):
    def __init__(self, dim):
        super(SEConv2dBlock, self).__init__()
        self.dim = dim
        self.conv = DepthWiseConv2d(dim, dim, 3, 1, 1)
        self.se = SElayer(dim)
...
class FrequencySeparate(nn.Module):
    def __init__(self, dim):
        ...
        self.dim = dim
        # blocks
        self.se1 = SelfAttentionTransformer(dim)
        self.se2 = SEConv2dBlock(dim)
        self.se3 = SelfAttentionTransformer(dim)
        self.se4 = SEConv2dBlock(dim)
