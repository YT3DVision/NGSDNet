...
class ConvFormer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, window_size):
        ...
        self.mixer = DepthWiseConv2d(in_channels, out_channels, kernel_size)
        self.mlp = Mlp(in_features=out_channels, hidden_features=out_channels*2, act_layer=nn.GELU, drop=0.1)
...
class DCT_Fusion(nn.Module):
    def __init__(self, in_channels):
        ...
        self.sfa1 = ConvFormer(in_channels, in_channels, 3, 8)
        self.sfa2 = ConvFormer(in_channels, in_channels, 3, 8)
        self.dwa1 = ConvFormer(in_channels, in_channels, 3, 8)
        self.dwa2 = ConvFormer(in_channels, in_channels, 3, 8)
