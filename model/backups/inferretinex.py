...
def main():
    ...
    # input0 = torch.randn(1, 3, scale, scale).cuda()
    # flops, params = profile(model, inputs=(input0, input0,))
    ...
    with torch.no_grad():
        ...
        for idx, img_name in enumerate(img_list):
            ...
            g3, g2, g1, g0, g_fuse, g_final, R_map, I_map = model(rgb, nir)
            ...
            R = R_map.data.squeeze(0)
            I = I_map.data.squeeze(0)

            g_final = np.array(transforms.Resize((scale, scale))(to_pil(g_final)))
            R = np.array(transforms.Resize((scale, scale))(to_pil(R)))
            I = np.array(transforms.Resize((scale, scale))(to_pil(I)))

            Image.fromarray(g_final).save(
                os.path.join(opt.result_path, img_name[:-4] + ".png")
            )
            Image.fromarray(R).save(
                os.path.join(opt.result_path, "R_" + img_name[:-4] + ".png")
            )
            Image.fromarray(I).save(
                os.path.join(opt.result_path, "I_" + img_name[:-4] + ".png")
            )
