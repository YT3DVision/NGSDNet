...
from model.module.Attention import AttentionLayer
...
class RetinexFusion(nn.Module):
    def __init__(self, dim, num_head=8, window_size=[8, 8]):
        ...
        self.se3 = SEConvBlock(dim, 3)

        self.fusion = SEConvBlock(2 * dim, 3)

        self.attn1 = AttentionLayer(dim, num_head, window_size)
        self.attn2 = AttentionLayer(dim, num_head, window_size)

        self.conv1 = BasicConv2d(dim, dim, 3, 1, 1)
        self.conv2 = BasicConv2d(dim, dim, 3, 1, 1)

    def forward(self, rgb, nir, rt_fea):
        rgb = self.se1(rgb)
        nir = self.se2(nir)
        rt_fea = self.se3(rt_fea)
        attn1 = self.attn1(rgb, nir*rt_fea, nir*rt_fea)
        attn2 = self.attn2(nir, rgb*rt_fea, rgb*rt_fea)
        rgb = rgb + self.conv1(attn1)
        nir = nir + self.conv2(attn2)
        fused = self.fusion(torch.cat([rgb, nir], 1))
        fused = torch.sigmoid(fused)
        map0 = fused[:, 0:self.dim, :, :]
        map1 = fused[:, self.dim:, :, :]

        return rgb * map0 + nir * map1
...
class RetinexNet(nn.Module):
    def __init__(
        ...
        # mcc
        self.fusion1 = RetinexFusion(embed_dim * 8)
        self.fusion2 = RetinexFusion(embed_dim * 4)
        self.fusion3 = RetinexFusion(embed_dim * 2)
        self.fusion4 = RetinexFusion(embed_dim)
    ...
    def forward(self, x, nir):
        ...
        # mcc stage
        layer3 = self.fusion1(rgb_layer3, nir_layer3, rt_fea3)
        layer2 = self.fusion2(rgb_layer2, nir_layer2, rt_fea2)
        layer1 = self.fusion3(rgb_layer1, nir_layer1, rt_fea1)
        layer0 = self.fusion4(rgb_layer0, nir_layer0, rt_fea0)
